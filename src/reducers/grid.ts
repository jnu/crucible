 ...
 */
export const updateGridContent = (content: ReadonlyArray<GridCell>, clues: ReadonlyArray<Clue>, height: number, width: number) => {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (cellType === CellType.Content) {
            ...
            const nextIsOpenAcross = hasRightCell && (!rightCellType || rightCellType === CellType.Content);
            const hasAcrossClue = firstOpenAcross && nextIsOpenAcross;
            const acrossWord: number = (nextIsOpenAcross ?
            ...
            const nextIsOpenDown = hasBelowCell && (!belowCellType || belowCellType === CellType.Content);
            ...
            if (hasAcrossClue || hasDownClue) {
                const acrossClue = hasAcrossClue ? 
                  {across: '', acrossStartIdx: i} : {across: null, acrossStartIdx: null};
                const downClue = hasDownClue ?
                  {down: '', downStartIdx: i} : {down: null, downStartIdx: null};
 ...
 */
export const snapToBounds = ({ state, index }: {state: GridState, index: number}) => {
 ...
 */
export const rSetCursorDirection = (state: GridState, action: SetCursorDirection) => {
 ...
 */
export const rHideMenu = (state: GridState, _action: HideMenu) => ({...state, menuCell: null});
...
*/
export const rToggleSymmetricalGrid = (state: GridState, _action: ToggleSymmetricalGrid) => {
...
*/
export const rUpdatePuzzleInfo = (state: GridState, action: UpdatePuzzleInfo) => {
...
*/
export const rAutoFillGridUpdate = (state: GridState, action: AutoFillGridStatsUpdate) => {
...
*/
export const rAutoFillGridError = (state: GridState, action: AutoFillGridError) => {
  return {
    ...state,
    autoFilling: false,
    autoFillStatus: null,
 ...
 */
export const rAutoFillGridDismissError = (state: GridState, _action: AutoFillGridDismissError) => {
