...
const td = (p: string) =>
  require(p.replace('test-data', '../../../../test-data'));
...
const grid5x5_partialNoCluesWithMeta = td(
  'test-data/grid/5x5_partial_noClues_withMeta.json',
);
const grid5x5_partialCluesWithMeta = td(
  'test-data/grid/5x5_partial_cluesWithMeta.json',
);
...
describe('Crux: v0 read/write', () => {
  describe('empty grids', () => {
    ...
    it('reads binary data as empty content', () => {
      const {binStr, header} = getHeaderAndBinStr(
        'ABQpAAAAAAAAAAAAAAAAABv7+xX/eMZV/3jOg',
      );
  ...
  describe('partial grid; no clues', () => {
    it('writes partial grids without clues', () => {
      ...
      chai
        .expect(str)
        .to.equal('ABQtAAAAAAAAAAAAAAAAAAACGAIQhCQIChCY6EgUsAFf94xlX/eM6');
    });
    it('reads partial grids without clues', () => {
      const {binStr, header} = getHeaderAndBinStr(
        'ABQtAAAAAAAAAAAAAAAAAAACGAIQhCQIChCY6EgUsAFf94xlX/eM6',
      );
      const puzzle = read(binStr, header);
  ...
  describe('partial grid; no clues; with meta info', () => {
    it('writes partial grids without clues but with unicode meta data', () => {
      ...
      chai
        .expect(str)
        .to.equal(
          'ABQtAAAAAAAAACYA+AOgC4ACGAIQhCQIChCY6EgUsAMOGIGfDvHRlciBww7zFvMWCxJnCoeKAoEjDr8OfIHB1xbrFumxlIGnFoSB3xK/FgmQhQ29weXJpZ2h0IMKpIDIwMTYsIEpvZSBOdWRlbGzQmNC+0YHQuNGEINCd0YPQtNC10LvRjFf94xlX/eM6',
        );
    });
  ...
  describe('partial grid; partial clues; with meta', () => {
    it('writes partial grids with partial clues', () => {
      ...
      chai
        .expect(str)
        .to.equal(
          '',
        );
    });
    ...
    it('reads partial grids with complete clues', () => {
      ...
      chai.expect(puzzle).to.eql({
        ...
        clues: [
          ...
          {
            across: 'Six more!!!',
            down: 'Надеюсь что и читать по русски возможно',
          },
          ...
          {
            across: 'End of the first alphabet, start of the second',
            down: 'Alaska',
          },
          ...
          {
            across: null,
            down: 'I wonder how this compares to the other software',
          },
          ...
          {
            across: 'I thought clue writing would be the easy part!',
            down: null,
          },
          {across: 'Crosswords are fun', down: null},
          {
            across: null,
            down: "That's not much, given the time it takes to develop a program :D",
          },
          ...
          {
            across: 'I wish I could hit tab to move to the next clue',
            down: null,
          },
