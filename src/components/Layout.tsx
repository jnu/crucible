...
class LayoutView extends React.Component<LayoutViewProps> {
  ...
  render() {
    const {width, height, cellSize, viewportWidth} = this.props;
    ...
    const lowerHorizontTop =
      puzzleContainerHeight + MAR_BIG + PUZZLE_INFO_HEIGHT;
    ...
    const autoFillOverlay =
      this.props.autoFilling && this.renderAutoFillOverlay();
  ...
  renderAutoFillOverlay() {
    ...
    const content = error ? (
      ...
      <div>
        <Stat title="Elapsed time">
          {Math.round(status.elapsedTime)} seconds
        </Stat>
        ...
        <Stat title="Words solved">
          {status.totalWords - status.leftToSolve}
        </Stat>
